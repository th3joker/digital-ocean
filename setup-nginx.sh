...
echo -e '# NginX security rules trigger (Default: 4 errors bans for 24 hours)
...
mkdir /etc/csf/scripts;
mkdir /etc/csf/scripts/cloudflare;
echo "Please enter cloudflare api key:"
read -p "APIKey: " apikey;
echo "Please enter cloudflare api email address:"
read -p "API email: " apiemail;
echo "#!/bin/bash
key="$apikey"
email="$apiemail"">> /etc/csf/scripts/cloudflare/secret.sh;

#Add nginx log scanning to LFD
sed -i.bak '2409i \CUSTOM1_LOG = "/home/nginx/domains/*/log/*.log"\' /etc/csf/csf.conf;
sed -i.bak '664i \BLOCK_REPORT = "/etc/csf/scripts/cloudflare/block.sh"\' /etc/csf/csf.conf;
sed -i.bak '665i \UNBLOCK_REPORT = "/etc/csf/scripts/cloudflare/unblock.sh"\' /etc/csf/csf.conf;
